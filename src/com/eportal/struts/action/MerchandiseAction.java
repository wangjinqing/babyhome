...
public class MerchandiseAction extends ActionSupport implements ModelDriven<Merchandise>{
	...
	/** JasperReport报表输出所需属性定义 */
	List<Merchandise> ds_merList; //定义ds_merList对象装载所有商品数据
	String reportMan; //定义reportMan变量存放报表人参数值
	String reportDate; //定义reportDate变量存放报表日期参数值
	String reportFormat = "HTML"; //定义reportFormat变量存放报表格式参数值(如：PDF、HTML、XML、CSV、XLS)
	...
	public String browseMerchandise(){
		...
		merList = service.browseMerchandise(pageNo,pageSize);//调用业务逻辑组件取得指定页的商品列表
		//封装分页数据
		PageList pageList = new PageList();
		pageList.setPageNumber(pageNo);//设置当前页数
		pageList.setFullListSize(count);//设置总记录数
		pageList.setList(merList);//设置当前页列表
