...
import com.eportal.service.impl.MemberLevelServiceImpl;
...
public class SearchMerTag extends SimpleTagSupport{
	...
	public void doTag() throws JspException, IOException{
    	...
    	memberLevelService = (MemberLevelService)WebApplicationContextUtils.getRequiredWebApplicationContext(((PageContext)getJspContext()).getServletContext()).getBean("memberLevelService");    	
    	if (listtype==1){//最新商品列表
    		hql = "from Merchandise as a where a.status=1 and (a.merName like '%"+keyword+"%' or a.merModel like '%"+keyword+"%' or a.merDesc like '%"+Tools.escape(keyword)+"%') order by a.id desc";  		
    	}else{//促销商品列表
    		hql = "from Merchandise as a where a.status=1 and a.special=1 and (a.merName like '%"+keyword+"%' or a.merModel like '%"+keyword+"%' or a.merDesc like '%"+Tools.escape(keyword)+"%') order by (a.price-a.sprice)*100/a.price desc"; 
    	}
